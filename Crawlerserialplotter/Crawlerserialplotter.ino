...
//Servo1 parameters femur
//This servo moves up/ccw with increased angle 
...
//Servo2 parameters tibia
//This servo moves down/cw with increased angle
...
void setup(){
  ...
  Serial.println("LABEL,Current time,Started Time,Femur,Tibia,Reward");
...
void setPhysicalState(int action){
  ...
  float finalAngle2;
  if (action == 0){
    currentAngle1 = servo1.read();
    currentAngle2 = servo2.read();
    finalAngle1 = currentAngle1 + deltaTheta1;
    servo1.write(finalAngle1);
    ...
    currentAngle2 = servo2.read();
    currentAngle1 = servo1.read();
    finalAngle2 = currentAngle2 + deltaTheta2;
...
float getDeltaDistanceRolled(){
  ...
  if (abs(deltaDistance) < 10.0 || abs(deltaDistance) > 230.0 ){         //don't count noise .......|| (deltaDistance) < 0.0
    deltaDistance = 0.0;
  }
